...
get_shapes=function(matrixs_tbl,surface,alpha)
	...
	o={o[1]/opn,o[2]/opn,o[3]/opn}
	local final_shapes={}
	for si=1,#surface do
		dlayer=surface[si].dlayer or 0
		local points={}
		for pi = 1,#surface[si] do
			points[#points+1]=matrixs_tbl[surface[si][pi]]
		end
		if #points >= 3 and dlayer ~= -1 then
			n_vec=get_nvec(points,o)
			if n_vec[3] > 0 then
				l_theta=math.deg(math.acos(in_product(n_vec,M3D.light_vec)/(vec_len(n_vec)*vec_len(M3D.light_vec))))/90-M3D.n_light
				s_layer=1
			elseif n_vec[3] <= 0 and alpha ~= 0 then
				n_vec={-n_vec[1],-n_vec[2],-n_vec[3]}
				l_theta=(math.deg(math.acos(in_product(n_vec,M3D.light_vec)/(vec_len(n_vec)*vec_len(M3D.light_vec))))/90-M3D.n_light)/alpha
				s_layer=0
			else
				l_theta=1
				s_layer=0
			end
			if n_vec[3] > 0 or alpha ~= 0 or dlayer < -1 then
				if dlayer <= -2 then
					dlayer=math.abs(dlayer)-2
				end
				for pi = 1,#points do
				...
				points_dc(points)
				if #points >= 3 then
					final_shapes[#final_shapes+1]={text=to_shape(points),color=light_color(surface[si].color or M3D.default_color,M3D.light_c,l_theta),layer=s_layer+dlayer,clip=""}
...
move_3d=function(st,et,dt,matrix,surface,alpha,matrix_fun)
	...
	if tenv.j == tenv.maxj then
		if tn-ti >= 2 then
			...
			local m=mat.copy(matrix)
			shapes=get_shapes(matrix_fun(m,surface,ti,tn),surface,alpha)
			tenv.loopctl(2,#shapes+2)
		elseif tn-ti == 1 then
			ti=ti+1
			loop_i=0
			local m=mat.copy(matrix)
			shapes=get_shapes(matrix_fun(m,surface,ti,tn),surface,alpha)
			tenv.loopctl(2,#shapes+1)
		end
	end
	loop_i=loop_i+1
	tenv.retime("abs",tenv.line.start_time+st+dt*(ti-1),tenv.line.start_time+st+dt*ti)
	tenv.m3d=shapes[loop_i]
	tenv.relayer(layer+shapes[loop_i].layer)
	return ""
end
...
set_surface_shape=function(matrix,surface,surface_shape)
	...
	for i=1,#surface_shape do
		...
		local s_shape_tbl={si=#matrix+1,text=surface_shape[i].text,color=surface_shape[i].color or M3D.default_color,layer=surface_shape[i].layer or 1,s=surface_shape[i].name or true}
...
M3D.mat=mat
M3D.get_shapes=get_shapes
M3D.move_3d=move_3d
M3D.set_surface_shape=set_surface_shape
M3D.set_cube=set_cube
M3D.set_diamond=set_diamond
M3D.set_prism=set_prism
M3D.set_pyramid=set_pyramid
M3D.get_nvec=get_nvec
